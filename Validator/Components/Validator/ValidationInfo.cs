    ...
    partial class Validator
    {
        ...
        partial class ValidationInfo
        {
            ...
            public void Validate()
            {
                ...
                // RegularExpression
                if ((this.type & ValidationType.RegularExpression) == ValidationType.RegularExpression)
                {
                    ...
                    case ValidationDataType.String:
                        {
                            ...
                            if ((!EvaluateIsValid(value, lower, ValidationCompareOperator.GreaterThanEqual))
                             || (!EvaluateIsValid(value, upper, ValidationCompareOperator.LessThanEqual)))
                            {
                    ...
                    case ValidationDataType.Integer:
                        {
                            ...
                            if ((!int.TryParse(this.rangeOfLowerBound, out lower))
                             ...
                             || (!EvaluateIsValid(value, lower, ValidationCompareOperator.GreaterThanEqual))
                             || (!EvaluateIsValid(value, upper, ValidationCompareOperator.LessThanEqual)))
                            {
                                this.SetError(ValidationType.Range);
                                return true;
            ...
            private bool HasCustomError()
            {
                ...
                if (this.CustomValidation != null)
                {
                    ...
                    CustomValidation(this.control, args);
                    if (!args.Valid)
                    {
                ...
                switch (errorType)
                {
                    ...
                    case ValidationType.Compare:
                        if (!string.IsNullOrEmpty(this.compareMessage))
                        {
                            ...
                            message = message.Replace("{$CompareOperator}", this.CompareOperator.ToString());
            ...
            #region Compare
            private static bool EvaluateIsValid(string original, string destnation, ValidationCompareOperator op)
            {
                bool valid = false;
            ...
            private static bool EvaluateIsValid(int original, int destnation, ValidationCompareOperator op)
            {
                bool valid = false;
                switch (op)
                {
            ...
            private static bool EvaluateIsValid(double original, double destnation, ValidationCompareOperator op)
            {
                bool valid = false;
                switch (op)
                {
            ...
            private static bool EvaluateIsValid(DateTime original, DateTime destnation, ValidationCompareOperator op)
            {
                bool valid = false;
                switch (op)
                {
            ...
            private static bool EvaluateIsValid(decimal original, decimal destnation, ValidationCompareOperator op)
            {
                bool valid = false;
                switch (op)
                {
